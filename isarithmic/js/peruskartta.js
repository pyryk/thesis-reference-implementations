var peruskartta = {
  ...
  panned: false,
  markersFile: ['reittiopas-data.json','reittiopas-data.json'],
  ...
  positionMarkers: function() {
    var importances = this.markerImportances;
    var dots = $.when.apply($, _.map(this.markersFile, function(file) { return $.getJSON(file); })).then(function(results){
      var results = _.map(arguments, function(it) { return it[0] });
      var dots = _.chain(results).flatten().groupBy(function(it) { return it.lat + ',' + it.lng; }).map(function(it) {
        var totalWeight = _.reduce(it, function(memo, it, i) { return memo + importances[i]; }, 0);
        var average = _.reduce(it, function(memo, it, i) { return memo + it.time * importances[i]; }, 0) / totalWeight;
        return {lat: it[0].lat, lng: it[0].lng, time: average};
      }).value();
      return dots;
    });
